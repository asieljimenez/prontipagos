...
const form = document.querySelector("form.contenedor-universal");
const input = document.querySelector(".input-1");
const contenedor = document.getElementById("id-contenedor-tarjeta");

// Contenido inicial por defecto
contenedor.innerHTML = `
    <div class="contenedor-universal">
    <img src="./assets/img/lupa.png" alt="" title="" class="imagen-resultados">
    </div>
    <p style="width: 100%; text-align: center; font-size: clamp(17px, 1.5vw, 25px);">Realiza una búsqueda</p>
`;

// Contenedor de sugerencias dinámico
const sugerencias = document.createElement("ul");
sugerencias.style.position = "absolute";
sugerencias.style.top = "28px";
sugerencias.style.background = "#fff";
sugerencias.style.border = "1px solid #ccc";
sugerencias.style.width = "100%";
sugerencias.style.listStyle = "none";
sugerencias.style.margin = "0";
sugerencias.style.padding = "8px 0px 0px 0px";
sugerencias.style.zIndex = "1";
sugerencias.style.maxHeight = "200px";
sugerencias.style.overflowY = "auto";
sugerencias.style.display = "none"; // Oculto por defecto
sugerencias.style.borderRadius = "0px 0px 10px 10px";
input.parentNode.appendChild(sugerencias);

let vendedores = [];

try {
    const response = await fetch("./assets/json/vendedores.json");
...
input.addEventListener("input", () => {
    ...
    const coincidencias = vendedores.filter(v =>
...
form.addEventListener("submit", (e) => {
    ...
    if (!termino) {
    // Si no escribió nada, volvemos al estado inicial
    contenedor.innerHTML = `
        <div class="contenedor-universal">
        <img src="./assets/img/lupa.png" alt="" title="" class="imagen-resultados">
        </div>
...
if (qrParam) {
    const vendedorQR = vendedores.find(v =>
        v.nombre.toLowerCase() === qrParam.toLowerCase()
    );
    ...
    if (vendedorQR) {
        ...
        modal.innerHTML = `
            <h2 style="margin-bottom: 10px; color: #222;">✅ Vendedor verificado</h2>
            <p style="margin: 0; color: #555; font-size: 16px;">
                ${vendedorQR.nombre} es un vendedor verificado.
            </p>
        `;
        ...
        // Quitar ventana después de 3 segundos
        setTimeout(() => {
            overlay.remove();
        }, 3000);
    }
...
// --- Mostrar tarjeta desde URL ---
const nombreParam = urlParams.get("nombre");

if (nombreParam) {
    const vendedorURL = vendedores.find(v =>
        v.nombre.toLowerCase() === decodeURIComponent(nombreParam).toLowerCase()
    );

    if (vendedorURL) {
        input.value = vendedorURL.nombre;
        form.dispatchEvent(new Event("submit"));
    }
}
...
setTimeout(() => {
const ventana = document.getElementById('id-ventana-emergente');
if (ventana) {
    ventana.style.display = 'flex';
} else {
    console.warn('No se encontró el elemento con id "id-ventana-emergente".');
}
